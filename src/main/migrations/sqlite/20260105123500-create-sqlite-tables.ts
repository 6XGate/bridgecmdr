...
export async function up({ context }: MigrationParams<Kysely<unknown>>) {
  ...
  await context.schema.createIndex('images_hash').ifNotExists().on('images').column('hash').unique().execute()
  ...
  await context.schema
    ...
    .addCheckConstraint('chk_ties_signed_integer_input_channel', sql`input_channel BETWEEN -2147483648 AND 2147483647`)
    .addCheckConstraint(
