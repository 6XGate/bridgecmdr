...
export const useTieRepository = memo(() => {
  ...
  async function all(): Promise<Tie[]> {
    return await db
      .selectFrom('ties')
      .selectAll()
      .execute()
  ...
  async function findById(id: UUID): Promise<Tie | null> {
    return await db
      .selectFrom('ties')
      .selectAll()
  ...
  async function insert(payload: NewTie): Promise<Tie> {
    return await db
      .insertInto('ties')
  ...
  async function update(id: UUID, payload: TieUpdate): Promise<Tie> {
    return await db
      .updateTable('ties')
  ...
  async function deleteById(id: UUID): Promise<Tie> {
    return await db
      .deleteFrom('ties')
      .where('id', '=', fromUuidString(id))
      .returningAll()
