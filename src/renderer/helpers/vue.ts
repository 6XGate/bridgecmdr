...
export const fromSchema = <Schema extends z.ZodTypeAny> (schema: Schema) => {
  ...
  if ('removeDefault' in schema) {
    return { validator, default: () => schema.parse(undefined) as unknown } as unknown as
      ExtractPropOptionsFromZod<Schema>
  }

  if (schema.isOptional()) {
    return { validator, default: schema.isNullable() ? null : undefined } as unknown as
      ExtractPropOptionsFromZod<Schema>
  }

  if (schema.isNullable()) {
    return { validator, default: null } as unknown as
      ExtractPropOptionsFromZod<Schema>
  }

  return { validator, required: true } as unknown as
      ExtractPropOptionsFromZod<Schema>
}
