...
export type TypedEventConstructor<E extends string, I extends EventInit> = new (type: E, init?: I) => TypedEvent<E>

export type TypedEventListener<Type extends string, Event extends TypedEvent<Type>> = (event: Event) => void
...
export type Listener<Callback extends (ev: Event) => void> = TypedEventListenerOrListenerObject<
  Parameters<Callback>[0]['type'],
...
export interface TypedEventTarget<Events extends EventMap> extends EventTarget {
  addEventListener: <E extends keyof Events>(
    type: E,
    listener: Listener<Events[E]> | null,
...
export interface TypedEventTargetEx<Events extends EventMap> extends TypedEventTarget<Events> {
  ...
  once: <E extends keyof Events>(
    type: E,
    listener: Listener<Events[E]> | null,
    options?: Omit<AddEventListenerOptions, 'once'> | boolean
  ) => void
...
const useTypedEventTarget = <Events extends EventMap>(existing?: EventTarget) => {
  ...
  return {
    ...
    once: (type, listener, options?) => {
      if (typeof options === 'boolean') addEventListener(type, listener, { capture: options, once: true })
      else addEventListener(type, listener, { ...options, once: true })
    }
