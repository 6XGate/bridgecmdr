...
const enableAutoStart = async () => { await settings.enableAutoStart(); autoStartEnabled.value = true }
const disableAutoStart = async () => { await settings.disableAutoStart(); autoStartEnabled.value = false }
const toggleAutoStart = async () => { autoStartEnabled.value ? await disableAutoStart() : await enableAutoStart() }
...
<template>
  <Page v-slot="{ toolbar, scrolled }">
    ...
    <VList v-scroll.self="scrolled" bg-color="transparent">
      <VListItem :title="t('option.autoStart')" lines="two"
                 :subtitle="t('description.autoStart')"
                 :prepend-icon="mdiRefreshAuto" @click="toggleAutoStart">
        <template #append>
          <VDivider class="mr-3" vertical/>
          <VSwitch :model-value="autoStartEnabled" density="compact" hide-details
                   @click.prevent.stop="toggleAutoStart"/>
        </template>
      </VListItem>
      <OptionDialog v-model="settings.iconSize" :items="iconSizes" :title="t('label.iconSize')"
                    max-width="320" mandatory with-cancel>
        <template #activator="{ props }">
          <VListItem v-bind="props" :title="t('option.iconSize')" lines="two"
                     :subtitle="t('item.iconSize', { size: settings.iconSize })"
                     :prepend-icon="mdiViewDashboard"/>
        </template>
      </OptionDialog>
      <OptionDialog v-model="settings.colorScheme" :items="colorSchemeOptions"
                    :title="t('option.colorTheme')" max-width="320" mandatory with-cancel>
        <template #activator="{ props }">
          <VListItem v-bind="props" :title="t('option.colorTheme')" lines="two"
                     :subtitle="t(`theme.${settings.colorScheme}`)"
                     :prepend-icon="mdiThemeLightDark"/>
        </template>
      </OptionDialog>
      <OptionDialog v-model="settings.powerOffWhen" :items="powerOffWhenOptions"
                    :title="t('label.powerOffWhen')" max-width="480" mandatory with-cancel>
        <template #activator="{ props }">
          <VListItem v-bind="props" :title="t('option.powerOffWhen')" lines="two"
                     :subtitle="t(`powerOff.${settings.powerOffWhen}`)"
                     :prepend-icon="mdiPower"/>
        </template>
      </OptionDialog>
      <VListItem :title="t('option.powerOnSwitchesAtStart')" lines="two"
      ...
      <VListItem :title="t('option.closeApp')" lines="two"
                 :subtitle="t('description.closeApp')"
                 :prepend-icon="mdiExitRun" @click="closeApp"/>
    </VList>
  </Page>
</template>
