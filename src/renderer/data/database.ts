...
import { z } from 'zod'
import MainDownPlugin from '@/data/level'
...
export const DocumentId = z.string().uuid().transform(value => value.toUpperCase())

export type Database<Schema extends z.AnyZodObject> =
  ReturnType<ReturnType<typeof defineDatabase<Schema>>>
export type DocumentOf<Schema extends z.AnyZodObject> =
  & IterableElement<Awaited<ReturnType<Database<Schema>['all']>>>

export type NewOf<Schema extends z.AnyZodObject> =
  & PutDocument<Parameters<Database<Schema>['add']>[0]>

PouchDb.plugin(MainDownPlugin)
PouchDb.plugin(find)

export const defineDatabase =
  <Schema extends z.AnyZodObject> (name: string, rawSchema: Schema, ...indicesBlocks: Indices[]) =>
    createSharedComposable(() => {
      type Doc = z.output<Schema>
      type Raw = z.input<Schema>
      type PouchDatabase = InstanceType<typeof PouchDB<Doc>>

      const schema = rawSchema.and(z.object({
      ...
      const get = async (id: DocumentId, attachments = true) => {
        ...
        return await db.get<Doc>(id.toUpperCase(),
          attachments ? { attachments: true, binary: true } : { })
      }
