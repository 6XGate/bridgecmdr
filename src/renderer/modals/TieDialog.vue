...
import { kDeviceCanDecoupleAudioOutput, useDrivers, kDeviceSupportsMultipleOutputs } from '@/system/driver'
...
const props = defineProps({
  ...
  tie: { type: Object as PropType<DeepReadonly<NewTie>>, required: true }
})
...
const audioOutputChannel = computed({
  ...
  set: value => { v$.outputChannels.audio.$model = value }
})

const videoOutputChannel = computed({
  get: () => v$.outputChannels.video.$model as number,
  set: value => { v$.outputChannels.video.$model = value }
})
...
const driver = computed(() => (
  switcher.value != null
    ? drivers.items.find(d => d.guid === switcher.value?.driverId)
    : undefined))

const hasOutputChannel = computed(() =>
  Boolean((driver.value?.capabilities ?? 0) & kDeviceSupportsMultipleOutputs))
  ...
  <VCard :laoding="isBusy" v-bind="cardProps">
    ...
    <VCardText ref="body">
      <VForm :disabled="isBusy">
        <VSelect v-model="v$.switchId.$model" :label="t('label.switch')" :items="switches.items"
                 item-title="title" item-value="_id" :placeholder="t('placeholder.required')"
                 v-bind="getStatus(v$.switchId)"/>
        <div v-if="target.switchId != null" class="colg d-flex flex-wrap justify-start">
          <NumberInput v-model="v$.inputChannel.$model" class="flex-grow-0 w-300px"
                       :label="t('label.inputChannel')" v-bind="getStatus(v$.inputChannel)"
                       :min="1" :placeholder="t('placeholder.required')"/>
          <template v-if="hasOutputChannel">
            <NumberInput v-model="videoOutputChannel" class="flex-grow-0 w-300px"
                         :label="t('label.outputChannel')" v-bind="getStatus(v$.outputChannels.video)"
                         :min="1" :placeholder="t('placeholder.required')"/>
          </template>
        </div>
        <template v-if="canDecoupleAudio">
          ...
          <div class="colg d-flex flex-wrap justify-start">
            <NumberInput v-model="audioOutputChannel" class="flex-grow-0 w-300px"
                         :label="t('label.audioChannel')" v-bind="getStatus(v$.outputChannels.audio)"
                         :min="1" :placeholder="t('placeholder.required')"
